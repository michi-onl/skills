...
| Rollback | `python3 scripts/rollback.py <page_id>` (see `scripts/rollback.py`) |
| Button attributes | See `references/divi-attributes.md` |
| Performance audit | See `references/performance.md` |
| StuV site credentials | See `references/wordpress_stuv.md` |

## Authentication

**Check memory first.** For known sites, credentials live in `references/wordpress_stuv.md`.
...
2. Append `?et_core_page_resource_remove_all=1` to any page URL while logged in
